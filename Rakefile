...
task 'db:create_migration' do
  ...
  ActiveRecord::Migrator.migrations_paths.each do |director|
    next unless File.exists?(directory)
    migration_files = Pathname(director).children
    if duplicate = migration_files.find { |path| path.basename.to_s.include?(name) }
      puts "Another migration is alread named \"#{name}\": #{duplicate}."
      exit
    end
  end
